...
old_profiledef=$(grep -oP 'limalinux_v\d{2}\.\d{2}\.\d{2}' archiso/profiledef.sh | grep -oP 'v\d{2}\.\d{2}\.\d{2}' | head -1)
...
sed -i "s|^iso_label=\"limalinux_.*\"|iso_label=\"limalinux_$newversion\"|" archiso/profiledef.sh
